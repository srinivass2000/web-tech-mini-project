...
class HomeController extends AbstractController
{
    ...
    public function calender(Request $request): Response
    {
        ...
        if ($form->isSubmitted()) {
            ...
            return $this->redirectToRoute('slotlist', ['date' => $date]);
        }
    ...
    public function index(Request $request, $date = 'date'): Response
    {
        ...
        $slots = $this->getDoctrine()
            ->getRepository(Slot::class)
            ->findBy(['slot_date' => $dateobj]);
        $slotArray = [];

        foreach ($slots as $slot) {
            array_push($slotArray, [
                $slot->getId(),
                $slot->getSlotDate()->format('Y-m-d'),
                $slot->getSlotTime()->format('H:i'),
                $slot->getBooked(),
            ]);
        }
    ...
    public function showReviews(Request $request): Response
    {
        ...
        if ($form->isSubmitted()) {
            ...
            $commented_at = \DateTime::createFromFormat(
                'm/d/Y h:i:s a',
                $currentDate
            );
        ...
        foreach ($reviews as $review) {
            ...
            array_push($reviewsArray, [
                $review->getId(),
                $user,
                $review->getCommentedAt()->format('Y-m-d H:i:s'),
                $review->getRating(),
                $review->getCategory(),
                $review->getComment(),
                $review->getReply(),
